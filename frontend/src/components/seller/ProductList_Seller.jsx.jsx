...
function ProductLists() {
  ...
  //     }));
  //   } catch (err) {
  //     console.log("Lỗi khi tải review:", err);
  //   }
  // };
  ...
  const reloadProducts = async () => {
    try {
      setLoading(true); // Thêm dòng này để hiện skeleton đẹp hơn khi reload
      const response = await axios.get(`${API_URL}/products/others`, {
        headers: {
      ...
      // Nếu đang mở BottomSheet → cập nhật lại sản phẩm đang chọn
      if (selectedProduct) {
        const updatedProduct = list.find((p) => p.id === selectedProduct.id);
        if (updatedProduct) {
          setSelectedProduct({
      ...
      setLoading(false);
      alert("Không tải lại được danh sách sản phẩm. Vui lòng thử lại!");
    }
  };
      ...
      <div className="min-h-screen bg-[#ffffff] p-8">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-5xl font-bold text-center mb-10 text-[#00214d]">
            Danh Sách Sản Phẩm Cuar Seller
          </h1>
    ...
    <div className="min-h-screen bg-[#ffffff]">
      ...
      {showCart && (
        <div className="fixed inset-0 bg-black/40 z-50 flex justify-center items-start pt-20">
          <div className="bg-white rounded-2xl w-11/12 max-w-3xl p-6 shadow-lg relative">
            <button
